#!/usr/bin/env node
/**
 * release-v1.4.mjs — Release orchestration script for SOA-Harness v1.4.0.
 *
 ...
 *
 * Differences from release-v1.3.mjs:
 *   - TARGET_VERSION bumped 1.3.0 → 1.4.0
 *   - PACKAGES list unchanged from v1.3 (no new packages in v1.4.0)
 *   - Spec changes are normative-additive only:
 *     - §18.5.6 Framework Reservations (Informative) — first reservation
 *       entry: "crewai" (no first-party adapter planned for v1.x line)
 *     - §17.2.3.2 Reserved-tokens registry mechanism (Normative) — defines
 *       admission regex, entry shape, lifecycle, governance; the registry
 *       file artifact itself is deferred to v1.4.1+
 *     - §17.2.3 emitter convention paragraph (Informative) — `x-` prefix
 *       SHOULD for non-registered tokens; RFC 6648 acknowledged
 *     - §14.6.4 wording cleanup — Adapter Deviation Protocol now
 *       framework-agnostic (LangGraph + non-LangGraph paths both addressed)
 *
 * USAGE
 *   node release-v1.4.mjs --impl-root <path> [--registry <url>] [--dry-run]
 *
 * EXAMPLES
 *   Staging (Verdaccio):
 *     node release-v1.4.mjs --impl-root ../soa-harness-impl --registry http://localhost:4873/
 *
 *   Production (npm):
 *     node release-v1.4.mjs --impl-root ../soa-harness-impl
 *     (uses the default registry from ~/.npmrc)
 *
 *   Dry run (no actual publish):
 *     node release-v1.4.mjs --impl-root ../soa-harness-impl --dry-run
 *
 * PREREQUISITES
 ...
 *   4. All 11 packages at 1.4.0 in impl/packages/* (separate impl-side
 *      pin-bump commit per repo CLAUDE.md "spec lands first" rule)
 ...
 *   - Pre-flight check validates that NO package@1.4.0 already exists on
...
function main() {
  ...
  console.log("Next: gh release create v1.4.0 per repo.");
}
