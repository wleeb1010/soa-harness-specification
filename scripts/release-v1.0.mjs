#!/usr/bin/env node
/**
 * release-v1.0.mjs — Release orchestration script for SOA-Harness v1.0.0.
 *
 * Publishes all 8 packages in dependency order to npm (or a custom registry
 * for staging), verifies each publish, and emits a JSON log of the sequence.
 *
 * USAGE
 *   node release-v1.0.mjs --impl-root <path> [--registry <url>] [--dry-run]
 *
 * EXAMPLES
 *   Staging (Verdaccio):
 *     node release-v1.0.mjs --impl-root ../soa-harness-impl --registry http://localhost:4873/
 *
 *   Production (npm):
 *     node release-v1.0.mjs --impl-root ../soa-harness-impl
 *     (uses the default registry from ~/.npmrc)
 *
 *   Dry run (no actual publish):
 *     node release-v1.0.mjs --impl-root ../soa-harness-impl --dry-run
 *
 * PREREQUISITES
 *   1. MANIFEST.json.jws is signed with the real release key (Phase 2e)
 *   2. Fresh granular npm token is active in ~/.npmrc (Phase 3a)
 *   3. Phase 2f Verdaccio dry-run passed
 *   4. All 8 packages at 1.0.0 in impl/packages/* (Phase 2a)
 ...
 *   - Pre-flight check validates that NO package@1.0.0 already exists on
...
// runner consumes them, memory backends consume runner, adapter consumes
// runner + langgraph, scaffold consumes runner + memory-mcp-sqlite.
const PACKAGES = [
  ...
  if (!existsSync(manifestJwsPath)) {
    ...
    if (/^[A-Za-z0-9_-]+\.\.0{50,}$/.test(jws)) {
      failures.push(
        ...
        `Sign with the real release key via Phase 2e before running this script.`,
      );
    }
  }
...
function main() {
  ...
  console.log("Next: gh release create v1.0.0 per repo (Phase 3f).");
}
