#!/usr/bin/env node
/**
 * release-v1.3.1.mjs — Release orchestration script for SOA-Harness v1.3.1.
 *
 ...
 *
 * USAGE
 *   node release-v1.3.1.mjs --impl-root <path> [--registry <url>] [--dry-run]
 *
 * EXAMPLES
 *   Staging (Verdaccio):
 *     node release-v1.3.1.mjs --impl-root ../soa-harness-impl --registry http://localhost:4873/
 *
 *   Production (npm):
 *     node release-v1.3.1.mjs --impl-root ../soa-harness-impl
 *     (uses the default registry from ~/.npmrc)
 *
 *   Dry run (no actual publish):
 *     node release-v1.3.1.mjs --impl-root ../soa-harness-impl --dry-run
 *
 * PREREQUISITES
 ...
 *   4. All 9 packages at 1.3.1 in impl/packages/*
 ...
 *   - Pre-flight check validates that NO package@1.3.1 already exists on
...
function main() {
  ...
  console.log("Next: gh release create v1.3.1 per repo.");
}
