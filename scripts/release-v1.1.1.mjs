#!/usr/bin/env node
/**
 * release-v1.1.1.mjs — Release orchestration script for SOA-Harness v1.1.1.
 *
 * Publishes all 9 packages in dependency order to npm (or a custom registry
 * for staging), verifies each publish, and emits a JSON log of the sequence.
 *
 * Differences from release-v1.1.mjs:
 *   - TARGET_VERSION bumped 1.1.0 → 1.1.1
 *   - Purpose: v1.1.1 patch release (L-64 Debt #7) — bumps
 *     PINNED_SPEC_COMMIT in @soa-harness/schemas from the stale
 *     68b34f1 (M7-week-1 commit) to 2184a32 (v1.1.0 spec tag
 *     target); wires governance.pinnedSpecCommit into the scaffold
 *     templates so create-soa-agent output surfaces spec_commit_sha
 *     at /version. No MANIFEST regen — the v1.1.0 signed MANIFEST
 *     remains authoritative for both v1.1.0 and v1.1.1 (no normative
 *     spec changes between them).
 *
 * USAGE
 *   node release-v1.1.1.mjs --impl-root <path> [--registry <url>] [--dry-run]
 *
 * EXAMPLES
 *   Staging (Verdaccio):
 *     node release-v1.1.1.mjs --impl-root ../soa-harness-impl --registry http://localhost:4873/
 *
 *   Production (npm):
 *     node release-v1.1.1.mjs --impl-root ../soa-harness-impl
 *     (uses the default registry from ~/.npmrc)
 *
 *   Dry run (no actual publish):
 *     node release-v1.1.1.mjs --impl-root ../soa-harness-impl --dry-run
 *
 * PREREQUISITES
 ...
 *   4. All 9 packages at 1.1.1 in impl/packages/*
 ...
 *   - Pre-flight check validates that NO package@1.1.1 already exists on
...
function main() {
  ...
  console.log("Next: gh release create v1.1.1 per repo.");
}
