...
Plan-evaluator Finding D surfaced at M2 Week 2 close: impl's `RUNNER_CARD_FIXTURE` loader performs a MANIFEST digest check (L-21), which correctly refuses to serve a tampered fixture. That same check made `SV-SESS-09` live-testing impossible: the validator couldn't swap the served card to trigger card_version drift without failing the digest check first.
...
- `security.trustAnchors` — identical. Both fixtures validate against the same handler keypair (L-24) and same placeholder trust anchor (L-21).
...
- Core §12.5 Trigger Points (L-29 — when resume fires)
